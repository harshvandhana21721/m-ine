...
function AppModal({ app, onClose, onCreated }: {
  ...
  async function handleSave(e: React.FormEvent) {
    ...
    try {
      ...
      const url = isEdit ? `/api/apps/${encodeURIComponent(app!.appId.trim().toUpperCase())}` : "/api/apps";
      const method = isEdit ? "PATCH" : "POST";
      const body = isEdit ? { name, pin: cleanPin, status } : { appId: cleanAppId, name, pin: cleanPin, status };
