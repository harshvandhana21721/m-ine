...
import { DEFAULT_APP_ID, DEFAULT_APP_NAME, DEFAULT_APP_PIN, localDb, normalizeAppId, normalizePin } from "../lib/local-db";
...
function autoDisableIfExpired(appId: string): void {
  const app = localDb.getApp(normalizeAppId(appId));
...
router.get("/apps/:appId", (req, res) => {
  ...
  const app = localDb.getApp(normalizeAppId(req.params.appId));
...
router.post("/apps", (req, res) => {
  ...
  if (!normalizedAppId || !name?.trim()) { res.status(400).json({ error: "appId and name are required" }); return; }
  try {
    const row = localDb.createApp({ appId: normalizedAppId, name, pin: normalizePin(pin), status });
...
router.patch("/apps/:appId", (req, res) => {
  ...
  if (pin !== undefined) updates.pin = normalizePin(pin);
  if (status !== undefined) updates.status = status;
  if (Object.keys(updates).length === 0) { res.status(400).json({ error: "No fields to update" }); return; }
  const targetAppId = normalizeAppId(req.params.appId);
  let row = localDb.updateApp(targetAppId, updates);
  if (!row && targetAppId === DEFAULT_APP_ID) {
    try { localDb.createApp({ appId: DEFAULT_APP_ID, name: DEFAULT_APP_NAME, pin: DEFAULT_APP_PIN, status: "active" }); } catch {}
    row = localDb.updateApp(targetAppId, updates);
  }
...
router.delete("/apps/:appId", (req, res) => {
  const row = localDb.deleteApp(normalizeAppId(req.params.appId));
  if (!row) { res.status(404).json({ error: "App not found" }); return; }
  res.json({ ok: true });
});
...
router.post("/apps/:appId/verify-pin", (req, res) => {
  ...
  if (normalizePin(app.pin) !== normalizePin(pin)) { res.status(401).json({ error: "Wrong PIN" }); return; }
