...
export function normalizePin(pin: string | undefined): string {
  const normalized = String(pin ?? DEFAULT_APP_PIN).trim();
  return normalized || DEFAULT_APP_PIN;
}
...
class LocalDb {
  ...
  getApp(appId: string): AppRow | undefined {
    ...
    return this.data.apps.find((a) => normalizeAppId(a.appId) === normalizedAppId);
  }
  createApp(input: { appId: string; name: string; pin?: string; status?: string }): AppRow {
    const appId = normalizeAppId(input.appId);
    if (this.getApp(appId)) throw new Error("APP_EXISTS");
    const row: AppRow = { id: this.data.meta.nextAppId++, appId, name: input.name.trim(), pin: normalizePin(input.pin), status: input.status ?? "active", createdAt: new Date().toISOString() };
  ...
  updateApp(appId: string, updates: Partial<Pick<AppRow, "name" | "pin" | "status">>): AppRow | undefined {
    ...
    if (updates.pin !== undefined) app.pin = normalizePin(updates.pin);
    if (updates.status !== undefined) app.status = updates.status;
    this.save(); return app;
  }
  deleteApp(appId: string): AppRow | undefined {
    const normalizedAppId = normalizeAppId(appId);
    const idx = this.data.apps.findIndex((a) => normalizeAppId(a.appId) === normalizedAppId); if (idx < 0) return undefined;
