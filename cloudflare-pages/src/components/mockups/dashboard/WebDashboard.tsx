  ...
  async function handleChange(e: React.FormEvent) {
    ...
    try {
      const verR = await fetch(`/api/apps/${appId}/verify-pin`, {
        method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ pin: oldPin.trim() }),
      });
      if (!verR.ok) { setErr("Current PIN is wrong."); return; }
      const cleanNewPin = newPin.trim();
      if (cleanNewPin.length < 4) { setErr("New PIN must be at least 4 characters."); return; }
      if (cleanNewPin !== newPin2.trim()) { setErr("PINs do not match."); return; }
      const patchR = await fetch(`/api/apps/${encodeURIComponent(appId.trim().toUpperCase())}`, {
        method: "PATCH", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ pin: cleanNewPin }),
      });
      if (!patchR.ok) { const j = await patchR.json().catch(() => ({})); setErr((j as { error?: string }).error ?? "Could not change PIN."); return; }
